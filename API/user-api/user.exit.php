...
$get_user_access = trim(urldecode(addslashes(htmlspecialchars($_GET['user_token']))));
$response_status = 'no';
if (!empty($get_user_id) && !empty($get_user_access)) {
    ...
    if (mysqli_num_rows($check_user_status) > 0 && !empty($check_user_status_array['user_token'])) {
        http_response_code(200);
        $bd_query = mysqli_query($db_connect, ("UPDATE `users` SET user_token = '' WHERE user_id = '$get_user_id'"));
        $response_status = 'ok';
    } else {
